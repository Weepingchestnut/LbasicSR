...
def img2windows(img, H_sp, W_sp):
    ...
    """
    B, C, H, W = img.shape                                              # torch.Size([1, 90, 64, 64])
    img_reshape = img.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)      # torch.Size([1, 90, 1, 64, 32, 2])
    img_perm = img_reshape.permute(0, 2, 4, 3, 5, 1).contiguous().reshape(-1, H_sp * W_sp, C)    # torch.Size([1, 1, 32, 64, 2, 90]) --> torch.Size([32, 128, 90])
    return img_perm
...
class Attention_axial(nn.Module):
    ...
    def im2win(self, x, H, W):
        B, N, C = x.shape       # torch.Size([1, 4096, 90])
        x = x.transpose(-2, -1).contiguous().view(B, C, H, W)       # torch.Size([1, 90, 64, 64])
        x = img2windows(x, self.H_sp, self.W_sp)
        x = x.reshape(-1, self.H_sp* self.W_sp, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()       # torch.Size([32, 3, 128, 30])
        return x
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        B, L, C = q.shape                                           # torch.Size([1, 4096, 90])
        assert L == H * W, "flatten img_tokens has wrong size"

        # partition the q,k,v, image to window
        q = self.im2win(q, H, W)    # torch.Size([32, 3, 128, 30])
        k = self.im2win(k, H, W)    # torch.Size([32, 3, 128, 30])
        v = self.im2win(v, H, W)    # torch.Size([32, 3, 128, 30])

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))  # B head N C @ B head C N --> B head N N; torch.Size([32, 3, 128, 128])
...
class Attention_regular(nn.Module):
    ...
    Args:
        ...
        position_bias (bool): The dynamic relative position bias. Default: True
    """
    def __init__(self, dim, resolution, idx, split_size=[2,4], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True):
        super().__init__()
        self.dim = dim
        self.dim_out = dim_out or dim
        self.resolution = resolution
...
class CATB_axial(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        B, L, C = x.shape
        assert L == H * W, "flatten img_tokens has wrong size"
        img = self.norm1(x)         # torch.Size([1, 4096, 180])
        qkv = self.qkv(img).reshape(B, -1, 3, C).permute(2, 0, 1, 3) # 3, B, HW, C; torch.Size([1, 4096, 540]) --> torch.Size([1, 4096, 3, 180]) --> torch.Size([3, 1, 4096, 180])
        # v without partition
        ...
        if self.shift_size > 0:
            ...
            # V-Rwin
            x1 = self.attns[0](qkv[:,:,:,:C//2], H, W).view(B, L, C//2).contiguous()    # torch.Size([3, 1, 4096, 90]) --> 
            # H-Rwin
...
class CAT(nn.Module):
    ...
    def forward_features(self, x):
        _, _, H, W = x.shape
        x_size = [H, W]
        x = self.before_RG(x)       # torch.Size([1, 4096, 180])
...
if __name__ == '__main__':
    ...
    model = CAT(upscale=2,
                ...
                block_name = 'CATB_regular',      # [CATB_regular, CATB_axial]
