...
from lbasicsr.archs.as_upsample import input_matrix_wpn, Pos2Weight, MetaUpscale
...
class STAN(nn.Module):
    def __init__(self,
        ...
        # ========= position to weight =======
        self.P2W = Pos2Weight(inC=num_feat)
    ...
    def forward(self, x, output_last_fea=None):
        ...
        scale_coord_map, mask = input_matrix_wpn(h, w, self.scale)
        scale_coord_map = scale_coord_map.to(x.device)
        local_weight = self.P2W(
            scale_coord_map.view(scale_coord_map.size(1), -1))  # (outH*outW, outC*inC*kernel_size*kernel_size)
        outH, outW = int(h * self.scale + 0.5), int(w * self.scale + 0.5)
        # ===================================================================================
        ...
        # sr = self.tail(x)
        # ================================ meta-upscale ==================================
        sr = self.metaupscale(x, self.scale, local_weight)
        sr = torch.masked_select(sr, mask.to(sr.device))
        sr = sr.contiguous().view(b, 3, outH, outW)
        # ================================================================================
