...
class STAN(nn.Module):
    def __init__(self,
        ...
        # self.ref_frame_ex = nn.Conv2d(num_in_ch, num_feat, kernel_size=9, stride=1, padding=9//2)
        self.ref_frame_ex = nn.Conv2d(num_in_ch, num_feat, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
    ...
    def forward(self, x, output_last_fea=None):
        ...
        for i, rg in enumerate(self.RG):
            ...
            x = rg(x)
            if (i + 1) % self.K == 0:
                x = self.sa_adapt[i](x, self.scale[0], self.scale[1])
            x = x + self.gamma * share_source
        x = self.conv_last(x)
        x += share_source

        # ---------- arbitrary scale upsampling -----------------
        sr = self.sa_upsample(x, self.scale[0], self.scale[1])
        sr = self.tail(sr)
        # =======================================================
...
if __name__ == '__main__':
    ...
    # nohup python -u
    # lbasicsr/train.py -opt options/train/STAN/train_STAN+tada+asup_Vimeo90K_BIx4.yml 2>&1 &
