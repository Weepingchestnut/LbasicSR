...
class DynamicUpsamplingFilter(nn.Module):
    ...
    def __init__(self, filter_size=(5, 5)):
        ...
        filter_prod = np.prod(filter_size)      # 25
    ...
    def forward(self, x, filters):
        ...
        """
        n, filter_prod, upsampling_square, h, w = filters.size()    # torch.Size([1, 25, 16, 4, 4])
        kh, kw = self.filter_size
        # ----------------------------------
        # x: torch.Size([batch, C=3, h, w])
        # self.expansion_filter: torch.Size([3*kh*kw, 1, kh, kw])
        # ----------------------------------
        expanded_input = F.conv2d(
            x, self.expansion_filter.to(x), padding=(kh // 2, kw // 2), groups=3)  # (batch, 3*filter_prod, h, w)
        expanded_input = expanded_input.view(n, 3, filter_prod, h, w).permute(0, 3, 4, 1, 2)  # (batch, h, w, 3, filter_prod)
        filters = filters.permute(0, 3, 4, 1, 2)  # (batch, h, w, filter_prod, upsampling_square]
        out = torch.matmul(expanded_input, filters)  # (batch, h, w, 3, upsampling_square)
...
class DUF(nn.Module):
    ...
    def forward(self, x):
        ...
        # residual image
        res = self.conv3d_r2(F.relu(self.conv3d_r1(x), inplace=True))   # torch.Size([b, 3*scale^2=48, 1, h, w])

        # filter -----------------------------------------------------------
        #   self.conv3d_f1(x): b x 256 x 1 x h x w --> b x 512 x 1 x h x w
        #   conv3d_f2: b x 512 x 1 x h x w --> b x (k*k*scale^2) x 1 x h x w
...
if __name__ == '__main__':
    ...
    # print(
    #     "DUF have {:.3f}M parameters in total".format(sum(x.numel() for x in net.parameters()) / 1000000.0))

    input = torch.rand(8, 7, 3, 4, 4).to(device)
    # get_flops(net, [7, 3, 180, 320])
