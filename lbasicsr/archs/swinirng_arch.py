...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        mask_windows, num_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1
    ...
    def forward(self, x, x_size):
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # [B*wh*ww, WH*WW, D], Re-embedding
...
class RSTB(nn.Module):
    ...
    def flops(self, resolutions):
        flops = 0
        flops += self.residual_group.flops(resolutions)
        H, W = resolutions
        flops += H * W * self.dim * self.dim * 9
        flops += self.patch_embed.flops(resolutions)
        flops += self.patch_unembed.flops(resolutions)
